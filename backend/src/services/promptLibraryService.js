      ...
      prompts: [
        {
          ...
          system: 'You are an expert maintenance documentation specialist. Extract inspection points with detail including acceptance criteria, tolerances, and notes. Keep responses EXTREMELY BRIEF. Item names: 3 words max. Inspection points: 1 sentence, 10 words max. Notes: 5 words max. Return ONLY valid JSON array.',
          userTemplate: `Based on the following maintenance manual excerpts:\n\n{context}\n\nCreate a comprehensive inspection checksheet with these columns:\n- Item Name (3 words max, specific equipment/component)\n- Inspection Point (1 sentence, 10 words max)\n- Frequency (Daily/Weekly/Monthly/Quarterly/Annual)\n- Expected Status (brief, specific acceptance criteria)\n- Notes (5 words max, essential instructions only)\n\nCRITICAL: Keep response EXTREMELY BRIEF. Return ONLY a JSON array. Example:\n[\n  {\n    "itemName": "Hydraulic Pump Motor",\n    "inspectionPoint": "Check motor bearing temperature",\n    "frequency": "Weekly",\n    "expectedStatus": "≤ 70°C",\n    "notes": "Measure housing. Shutdown if > 80°C"\n  }\n]`,
          version: '1.0.0',
      ...
      prompts: [
        {
          ...
          system: 'You are an expert in extracting maintenance procedures from technical manuals. Extract procedural steps, inspection points, and maintenance tasks. Keep responses EXTREMELY BRIEF. Title: 5 words max. Overview: 1 sentence, 15 words max. Prerequisites: 2-3 items per category, each 2-3 words. Steps: max 2 per request, title 3 words, description 1 sentence 10 words max. Safety: max 2 items, 5 words each. Completion: max 2 items, 3 words each. Return ONLY valid JSON.',
          userTemplate: `Extract maintenance work instructions from this manual:\n\n{context}\n\nYour task: Find and extract ALL maintenance procedures including:\n- Inspection procedures (daily, weekly, monthly, annual checks)\n- Maintenance tasks (lubrication, adjustment, cleaning, replacement)\n- Service procedures (overhaul, calibration, testing, repairs)\n- Safety procedures (lockout/tagout, hazard warnings)\n- Troubleshooting steps\n\nExtraction Guidelines:\n1. Look for numbered steps, bullet points, or sequential procedures\n2. Extract specific measurements, torque specs, tolerances, and technical values\n3. Include tool requirements and part numbers mentioned\n4. Capture frequency/schedule information (daily, weekly, monthly, annual)\n5. Extract safety warnings and PPE requirements\n6. Include acceptance criteria and quality checks\n7. If multiple procedures exist, choose the most detailed one\n\nReturn ONLY valid JSON (no markdown, no explanations):\n{\n  "title": "Name of the procedure extracted (e.g., 'Monthly Hydraulic System Maintenance')",\n  "overview": "What this procedure does and when to perform it",\n  "frequency": "Daily/Weekly/Monthly/Quarterly/Annual/As-Needed",\n  "estimatedDuration": "Estimated time (e.g., '45 minutes')",\n  "prerequisites": {\n    "tools": ["Tool name with specification (e.g., 'Torque wrench 50-250 Nm')"],\n    "materials": ["Materials or parts needed (e.g., 'SAE 10W-30 oil, 2 quarts')"],\n    "safety": ["Required PPE and safety equipment"]\n  },\n  "steps": [\n    {\n      "stepNumber": 1,\n      "description": "Clear action statement",\n      "details": "Complete details with measurements, specifications, inspection criteria",\n      "warning": "Safety warning for this step (if applicable)"\n    }\n  ],\n  "safetyWarnings": ["Overall safety warnings and hazards"],\n  "completionChecklist": ["How to verify work was completed correctly"],\n  "notes": "Additional references or related information"\n}\n\nIMPORTANT: Extract actual content from the manual. Do not create generic procedures. If no clear procedure is found, extract inspection points or maintenance tasks as steps.`,
          version: '2.0.0',
